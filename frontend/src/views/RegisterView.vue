    ...
    <div class="w-full max-w-md px-3 sm:px-4">
      <div class="space-y-4 sm:space-y-6 md:space-y-8">
        ...
        <div class="text-center">
          ...
          <p class="text-slate-800 font-semibold text-sm sm:text-base">Bergabunglah dengan komunitas Night Stalkers</p>
        </div>

        <!-- Register Card -->
        <div class="rounded-2xl sm:rounded-3xl shadow-xl border border-slate-200 p-5 sm:p-6 md:p-8" style="background-color: #FCF8F8;">
          <form @submit.prevent="handleRegister" class="space-y-3 sm:space-y-4 md:space-y-6">
            <!-- Name Input -->
            <div class="space-y-1 sm:space-y-2">
              <label class="block text-xs sm:text-sm font-bold text-slate-900">Nama Lengkap</label>
              <input 
                v-model="name" 
                type="text" 
                placeholder="John Doe"
                class="w-full bg-gradient-to-b from-slate-50 to-slate-100 border-2 border-slate-200 rounded-lg sm:rounded-xl p-2 sm:p-3 md:p-4 focus:outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-200 transition-all duration-200 text-slate-900 placeholder:text-slate-700 font-medium text-xs sm:text-sm"
                required
              />
            </div>
            ...
            <div class="relative py-2 sm:py-3">
              ...
              <div class="relative flex justify-center text-xs sm:text-sm">
                <span class="px-2 text-slate-500" style="background-color: #FCF8F8;">atau</span>
              </div>
            </div>
            ...
            <router-link 
              to="/login"
              class="block w-full text-center bg-slate-100 hover:bg-slate-200 text-slate-700 py-2 sm:py-3 md:py-4 rounded-lg sm:rounded-xl font-bold text-sm sm:text-base uppercase tracking-wider transition-all duration-200"
            >
              Sudah Punya Akun?
            </router-link>
          </form>
